...
Inputs (doc/):
  ...
  zcn_sn_read_ceiling.csv     E2E read ceiling (optional; latency vs λ)
  zcn_sn_write_ceiling.csv    E2E write ceiling (optional; latency vs λ)

Outputs:
  zcn_sim_report.pdf   (page 1: topology + latency; page 2: utilisation)
  zcn_sim_report_p1.png
  zcn_sim_report_p2.png
"""
import csv
import os
...
PNG_P1 = os.path.join(HERE, "zcn_sim_report_p1.png")
PNG_P2 = os.path.join(HERE, "zcn_sim_report_p2.png")
...
def load_latency_sweep(path, lat_keys):
    ...
    lat_keys is an ordered list of candidate column names; the first present
    value per row wins (e.g. new read_plat, falling back to legacy read_flat).
    """
...
def draw_topology(ax):
    """6×7 CHI mesh with 2RN+2HN per router and 4 top-row SN/DDR nodes."""
    def rid(r, c):
        return r * COLS + c
    ...
    for r in range(ROWS):
        for c in range(COLS):
            ...
            if has_sn:
                sn_y = y + 0.58
                ax.plot([x, x], [y, sn_y], color=C_SN_EDGE, lw=1.4, zorder=2)
                ax.add_patch(Rectangle(
                    (x - sn_w / 2, sn_y - sn_h / 2), sn_w, sn_h,
                    facecolor=C_SN_FILL, edgecolor=C_SN_EDGE, lw=1.0, zorder=4))
                ax.text(x, sn_y, f"SN{sn_nodes[(r, c)]}", ha="center", va="center",
    ...
    ax.axis("off")
    ax.set_title(
        "Topology: 6×7 CHI mesh — 84 RN + 84 HN + 4 SN (DDR)\n"
        "SN @ R1/R2/R4/R5 (nodes 168–171), XY routing, link_latency=2",
...
def page_topology_latency(pdf, read_lat, write_lat):
    ...
    if plotted:
        ...
        # log scale helps when unstable points jump to hundreds of cycles
        ymax = max(
